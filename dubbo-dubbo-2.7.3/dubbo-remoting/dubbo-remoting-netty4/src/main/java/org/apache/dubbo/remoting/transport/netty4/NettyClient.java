	...
	protected void doOpen() throws Throwable {
		...
		bootstrap.handler(new ChannelInitializer() {
			...
			protected void initChannel(Channel ch) throws Exception {
				...
				//初始化客户端编解码器Adapter 根据SPI加载编解码器  运行时是 DubboCodec---> ExchangeCodec----> TelnetCodec
				NettyCodecAdapter adapter = new NettyCodecAdapter(getCodec(), getUrl(), NettyClient.this);
				ch.pipeline()//.addLast("logging",new LoggingHandler(LogLevel.INFO))//for debug
						.addLast("decoder", adapter.getDecoder()) //客户端  响应解码器
						.addLast("encoder", adapter.getEncoder()) //客户端  端请求编码器
						.addLast("client-idle-handler", new IdleStateHandler(heartbeatInterval, 0, 0, MILLISECONDS))
						.addLast("handler", nettyClientHandler); //客户端业务处理器
