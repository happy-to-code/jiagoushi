...
public class HeaderExchangeHandler implements ChannelHandlerDelegate {
	...
	void handleRequest(final ExchangeChannel channel, Request req) throws RemotingException {
		Response res = new Response(req.getId(), req.getVersion());//创建响应对象
		if (req.isBroken()) {
		...
		try {
			/**
			 * handler: DubboProtocol.requestHandler.reply 中处理请求   ********重点******
			 */
			...
			future.whenComplete((appResult, t) -> {
				try {
					if (t == null) {
						...
						res.setResult(appResult); // 结束时设置响应Result
					} else {
					...
					/**
					 * 当调用完成后将结果写回
					 * 发送响应数据，接下来就要到响应编码阶段了，响应编码之后真正发送数据
					 */
					channel.send(res);
