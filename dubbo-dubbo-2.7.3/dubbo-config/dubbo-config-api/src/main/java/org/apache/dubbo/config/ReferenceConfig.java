...
public class ReferenceConfig<T> extends AbstractReferenceConfig {
	...
	private void init() {
		...
		//***核心***  创建代理类 因此<dubbo:reference> 真正被注入到容器中的是代理对象
		ref = createProxy(map);
	...
	/**
	 * 1、构建以及合并 Invoker 实例
	 * 2、创建代理, 把Invoker通过动态代理转换成实现用户接口的动态代理引用
	 */
	private T createProxy(Map<String, String> map) {
		...
		if (shouldJvmRefer(map)) {
			...
			if (url != null && url.length() > 0) { // user specified URL, could be peer-to-peer address, or register center's address.
				...
				// if protocols not injvm checkRegistry *****基于注册中心的远程调用，重点分析这块******
				if (!LOCAL_PROTOCOL.equalsIgnoreCase(getProtocol())) {
					...
					if (CollectionUtils.isNotEmpty(us)) {
						for (URL u : us) {
							// registry://192.168.200.129:2181/org.apache.dubbo.registry.RegistryService?application=demo-consumer&dubbo=2.0.2&pid=6632&qos.port=33333&registry=zookeeper&timestamp=1622603399790
