...
public abstract class FailbackRegistry extends AbstractRegistry {
	...
	public void register(URL url) {
		...
		try {
			...
			// 执行注册核心方法 实现在 ZookeeperRegistry 中
			doRegister(url);
		} catch (Exception e) {
			Throwable t = e;
			...
			// Record a failed registration request to a failed list, retry regularly 失败后重试
	...
	public void subscribe(URL url, NotifyListener listener) {
		...
		try {
			...
			//url consumer://192.168.200.10/org.apache.dubbo.demo.DemoService?application=demo-consumer&category=providers,configurators,routers&check=false&dubbo=2.0.2&interface=org.apache.dubbo.demo.DemoService&lazy=false&methods=sayHello&pid=1432&qos.port=33333&side=consumer&sticky=false&timestamp=1622625652317
			doSubscribe(url, listener);// 子类实现 ZookeeperRegistry 中
		} catch (Exception e) {
			Throwable t = e;
